...
void compute_trajectory_position_feature(database& db, int& offset, float weight = 1.0f)
{
    for (int i = 0; i < db.nframes(); i++)
    {
        ...
        // std::cout<<db.bone_positions(i, 0)<<std::endl;
        // std::cout<<db.bone_positions(1)<<std::endl;
        // std::cout<<db.bone_positions(2)<<std::endl;




        // trajectory_pos0.y = findClosestY(terrain_y, trajectory_pos0.x, trajectory_pos0.z) - root_terrain_height;
        // trajectory_pos1.y = findClosestY(terrain_y, trajectory_pos1.x, trajectory_pos1.z) - root_terrain_height;
        // trajectory_pos2.y = findClosestY(terrain_y, trajectory_pos2.x, trajectory_pos2.z) - root_terrain_height;
