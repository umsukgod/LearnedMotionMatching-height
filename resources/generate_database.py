...
files = [
    ...
    ('obstacles1_subject1.bvh',            120, 5390),
    ('pushAndStumble1_subject5.bvh', 194,  351), 
    # Running
    ('run1_subject5.bvh',             90, 7086),
    # Walking
    ('walk1_subject5.bvh',            80, 7791),
    ('obstacles1_subject2.bvh',            120, 5390),
    ('obstacles1_subject5.bvh',            120, 5390),
]
...
for filename, start, stop in files:
    ...
    for mirror in [False, True]:
        ...
        # sim_position = np.array([1.0, 0.0, 1.0]) * global_positions[:,sim_position_joint:sim_position_joint+1]
        # sim_position[:,0,0] = signal.savgol_filter(sim_position[:,0,0], 31, 3, axis=0, mode='interp')
        # sim_position[:,0,2] = signal.savgol_filter(sim_position[:,0,2], 31, 3, axis=0, mode='interp')
        # sim_position[:,0,1] = signal.savgol_filter(sim_position[:,0,1], 31, 3, axis=0, mode='interp')
        sim_position = signal.savgol_filter(sim_position, 31, 3, axis=0, mode='interp')
        # sim_position[2] = signal.savgol_filter(sim_position, 31, 3, axis=0, mode='interp')
        ...
        # breakpoint()
        positions[:,1,1] -= sim_position[:,0,1]
        # positions[:,0,1] -= sim_position[:,0,1]
...
with open('./resources/database.bin', 'wb') as f:
